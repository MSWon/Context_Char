...
class Char2Vec():
    ...
    def initialize_char_embedding(self, embedding_size=20, vocab_size=94):
        
        self.char_embedding = tf.Variable(
                tf.random_normal([vocab_size, embedding_size], mean = 0.0, stddev = 1.0))   
...
class Char2BiLSTM():
    ...
    def FC_layer(self, final_state, wordembedding_size=64):
        
        with tf.variable_scope('FC_Weights', reuse = tf.AUTO_REUSE):
            self.W = tf.get_variable(name="W1", shape=[2 * self.lstm_units, wordembedding_size],
                                     dtype=tf.float32, initializer = tf.contrib.layers.xavier_initializer())
            self.b = tf.get_variable(name="b1", shape=[wordembedding_size], dtype=tf.float32,
                                     initializer=tf.zeros_initializer())
        
        logits = tf.matmul(final_state, self.W) + self.b
        return logits
